...
- **`vi_ros2/`** — ROS2 (Humble) Rust nodes: `vi_node` (Vi.action server, interface-equivalent to the ROS1 node) and `vi_planner` (a Nav2 `compute_path_to_pose` action server that replaces nav2_planner's planner_server: per-goal VI solve — default `frontier2d_sparse` — with a cached value function, then greedy policy rollout via `vi_reference::planner` into a `nav_msgs/Path`). Both call `vi_rs` (the u64 `vi_reference` solvers) and build via colcon in the Docker image.
