...
- **`vi_ros2/`** — ROS2 (Humble) Rust nodes: `vi_node` (Vi.action server, interface-equivalent to the ROS1 node), `vi_planner` (**one** node serving both `compute_path_to_pose` and `follow_path` from a **single** value function — replaces planner_server and controller_server together) and `vi_global_planner` (`compute_path_to_pose` only, but with `map_scale` and the out-of-core `frontier2d_sparse_compact` path for maps too large to solve densely; pair it with nav2_controller's controller_server). All call `vi_rs` (the u64 `vi_reference` solvers) and build via colcon in the Docker image.
