#!/usr/bin/env bash
# vi_lib の u64 高速ソルバ群ハーネス (vi_u64_bench) をビルドし、house.pgm 上で
# 各ソルバを走らせて value_<solver>.npy / policy_<solver>.npy / timing_<solver>.json を
# /results に出力する。ref と同じく ROS 非経由・単スレッド。
#
# vi_lib は std のみ依存。リポジトリ直下の .cargo/config.toml (ROS patch) を拾うと
...
cd /tmp
cargo build --release --manifest-path /workspace/vi_rs/Cargo.toml -p vi_lib --bin vi_u64_bench
BIN=$CARGO_TARGET_DIR/release/vi_u64_bench

PARAMS="${PARAMS:-/workspace/vi_compare/benches/house/params.yaml}"
SOLVERS="${SOLVERS:-reference frontier3d frontier2d frontier_stack block_refine pyramid_sweep prio_ls prio_lc}"
for s in $SOLVERS; do
  echo "== u64 solver: $s =="
  python3 /workspace/vi_compare/benches/house/vi_rs/u64_bench.py \
    "$s" \
    "$PARAMS" \
