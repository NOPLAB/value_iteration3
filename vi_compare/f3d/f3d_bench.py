#!/usr/bin/env python3
"""vi_rs Frontier3D 比較ドライバ。

ros2 bench_client / ref_bench と **完全に同一の** map_server 意味論で house.pgm を
OccupancyGrid 化し、その raw int8 (h*w, row-major) を Rust ハーネス `vi_f3d_bench` に渡す。
ハーネスが value_f3d.npy / policy_f3d.npy / timing_f3d.json を out_dir に書く。

ref_bench.py と入力生成 (to_occupancy) は完全一致させてあるので、f3d / ref / ros2 は
同一のゴール・penalty・transition で解かれる。

使い方:
  f3d_bench.py <params.yaml> <map.pgm> <out_dir> <vi_f3d_bench_bin>
"""
...
def to_occupancy(w, h, pgm, meta):
    """ros2 bench_client.to_occupancy / ref_bench.to_occupancy と同一: map_server 意味論 + flipud。
...
def main():
    ...
    occ_raw = os.path.join(out_dir, 'occ_f3d.raw')
    ...
    print('[f3d_bench] running:', ' '.join(cmd), flush=True)
    subprocess.run(cmd, check=True)
    try:
        os.remove(occ_raw)
    except OSError:
        pass
    print('[f3d_bench] done -> %s/{value_f3d,policy_f3d}.npy, timing_f3d.json' % out_dir)
