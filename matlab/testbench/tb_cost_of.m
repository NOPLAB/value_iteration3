...
%TB_COST_OF Unit tests for cost_of function.
    ...
    % Test 1: Normal addition includes the one-step base cost
    assert(cost_of(100, 50) == 151, 'Normal addition failed');
    ...
    % Test 4: Neighbor penalty is GOAL → treated as 0, but still adds one step
    assert(cost_of(100, PENALTY_GOAL) == 101, 'GOAL penalty failed');
    ...
    assert(cost_of(0, PENALTY_GOAL) == 1, 'Goal zero + GOAL penalty failed');
