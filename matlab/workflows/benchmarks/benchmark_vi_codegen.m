...
function row = build_row(impl, c, sw_m, t_m, mt_m, sw_c, t_c, mt_c, metrics)
    row = struct( ...
        'impl',                impl, ...
        'name',                c.name, ...
        'map_x',               c.map_x, ...
...
function print_markdown(rows, impl, title, iter_label)
    ...
    fprintf('\n### %s\n\n', title);
    fprintf('| name | size | %s | matlab ms | matlab ms/%s | codegen ms | codegen ms/%s | speedup | vmiss%% | maxabs |\n', ...
        iter_label, iter_label, iter_label);
    fprintf('|------|------|----:|----------:|--------------:|-----------:|---------------:|--------:|-------:|-------:|\n');
    for k = 1:numel(sel)
        r = sel(k);
        fprintf('| %s | %dx%d | %d | %.1f | %.2f | %.1f | %.2f | %.2fx | %.2f | %.0f |\n', ...
...
function write_csv(rows, path)
    ...
    for k = 1:numel(rows)
        r = rows(k);
        fprintf(fid, '%s,%s,%d,%d,%s,%d,%.6f,%.6f,%.0f,%d,%.6f,%.6f,%.0f,%.6f,%.6f,%.6f,%.0f\n', ...
