...
//! 配線するまで実機でも起きる (→ 2026-08-21 に vi_planner の
//! `belief_recovery` として配線済み)。
//!
//! ローカル再マッチ (2026-08-21): 追跡からの不一致 (一時破綻 or 誘拐検出) の
//! informed 再注入を**比較採用**に — 推定近傍 5 m だけの候補と全域候補を同じ
//! LUT で採点し、近傍最良が全域最良の 1/2 以上なら近傍だけを注入する
//! (belief.rs の LOCAL_REMATCH_*)。一時破綻の現場では近傍が真値スケール
//! (~0.7) を出していても、全域のどこかに単発スキャン 0.98 の偶然エイリアスが
//! ほぼ常にあり、旧来はそれへの全域注入 (= 復帰サイクル churn の源) が
//! 選ばれていた。絶対しきい値 0.5 は一度線を超えた誤アンカーを局所再注入で
//! 全域補正から保護して逆効果 (err max 130 m へ悪化)、厳しい比 0.8 は偶然
//! エイリアスに負けて不発 — 寛容な比 0.5 で **K4 ルート対照 (誘拐なし・
//! フルスタック) が初の reached** (1230 s、従来はどの構成でも TIMEOUT)。
//! 誘拐 K1〜K4 の検出は全滅復旧経路 (相対枝刈りの全滅 → アンダーフロー →
//! 全域マッチ) を通って比較分岐に入らないため、relock 4/4 =
//! 17.6/23.1/24.2/41.7 s は bit 一致で保存 (回帰で実測。K4 後半の再ロストでは
//! 比較分岐が 2 回発火しどちらも正しく全域を選んだ — local 0.45 vs 0.97)。
