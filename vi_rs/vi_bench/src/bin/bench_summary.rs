...
fn print_markdown(rows: &[CaseRow]) {
    println!("| case | solver | iters_or_sweeps | updates | total_ms | converged | mismatch |");
    println!("|------|--------|-----------------|---------|----------|-----------|----------|");
    for r in rows {
        println!(
            "| {} | {} | {} | {} | {:.3} | {} | {} |",
...
fn write_csv(path: &Path, rows: &[CaseRow]) -> std::io::Result<()> {
    ...
    writeln!(
        f,
        "case,solver,iters_or_sweeps,updates,total_ms,converged,mismatch"
    )?;
    for r in rows {
        writeln!(
            f,
...
fn main() -> ExitCode {
    ...
    // Smoke-mode overrides: keep CI cost trivial.
    if args.smoke {
        args.sizes = vec![8];
    ...
    for &size in &args.sizes {
        for type_str in &args.types {
            ...
            eprintln!(
                "  size={size} type={type_str} solver=reference iters={} updates={} ms={:.2} mismatch=0",
            ...
            for entry in registry.iter().skip(1) {
                ...
                eprintln!(
                    "  size={size} type={type_str} solver={solver_name} iters={} updates={} ms={:.2} mismatch={}",
    ...
    // fails the run. Skipped in smoke mode — with budget=1 several solvers
    // (notably PyramidSweep, whose internal coarse/refine schedule is not
    // capped by the outer budget) can finish before Reference's 1-sweep
    // pass propagates the goal far enough to agree. Smoke mode is a wiring
    ...
    for r in &rows {
        if r.mismatch > 0 && EXACT_SOLVERS.contains(&r.solver) {
            eprintln!(
                "WARNING: {} {} mismatch={}",
                r.case_label, r.solver, r.mismatch,
            );
