...
//! (`ValueIterator::value_iteration_worker`) を strict 固定点まで回すソルバ。
//! 旧 `solvers::reference_solve` の置き場で、`U64Solver::Reference` と
//! `U64Solver::StreamMimic` (行優先 sweep = 本家順そのもの) がここへ委譲する。
//!
//! 他の全ソルバの bit-exact 検証のオラクル (conformance テストの比較基準) は
//! この全走査の固定点。
