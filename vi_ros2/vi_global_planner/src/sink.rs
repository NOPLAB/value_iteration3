...
//! `vi_bench::bench_map` と同じ実装をプランナ側にも置く。value (u64 LE) と policy (i32 LE) を
//! 2 ファイルに分け、finalize 時は列連続で write、ロールアウト時は orig 単位で read する。
//!
...
mod tests {
    ...
    fn unwritten_cells_read_as_unreachable_and_roundtrip() {
        let dir = std::env::temp_dir().join("vi_global_planner_sink_test");
